struct Heap<T> {
    private var arr = [T]()
    private let compare: (T,T) -> Bool
    
    init(arr: [T] = [T](), compare: @escaping (T, T) -> Bool) {
        self.arr = arr
        self.compare = compare
    }
    
    mutating func enheap(_ newElement: T) {
        arr.append(newElement)
        
        var childIndex = arr.count - 1
        var parentIndex = (childIndex - 1) / 2
        
        while childIndex > 0 && compare(arr[parentIndex], arr[childIndex]) {
            arr.swapAt(childIndex, parentIndex)
            childIndex = parentIndex
            parentIndex = (childIndex - 1) / 2
        }
    }
    
    mutating func deheap() -> T? {
        if arr.isEmpty { return nil }
        if arr.count == 1 { return arr.removeLast() }
        
        let value = arr[0]
        arr[0] = arr.removeLast()
        
        var index = 0
        while true {
            let leftChildIndex = index * 2 + 1
            let rightChildIndex = leftChildIndex + 1
            
            var target = index
            if leftChildIndex < arr.count && compare(arr[target], arr[leftChildIndex]) {
                target = leftChildIndex
            }
            if rightChildIndex < arr.count && compare(arr[target], arr[rightChildIndex]) {
                target = rightChildIndex
            }
            if target == index { break }
            arr.swapAt(target, index)
            index = target
        }
        
        return value
    }
}

let nm = readLine()!.split(separator: " ").map { Int($0)! }
var bungi = readLine()!.split(separator: " ").map { Int($0)! }
var edges = [[(d: Int, w: Int)]](repeating: [], count: nm[0])
var weight = [Int](repeating: Int.max, count: nm[0])
var heap = Heap<(d: Int, w: Int)>{ $0.w > $1.w }

var result = -1

for _ in 0..<nm[1] {
    let input = readLine()!.split(separator: " ").map { Int($0)! }
    edges[input[0]].append((input[1], input[2]))
    edges[input[1]].append((input[0], input[2]))
}


heap.enheap((0,0))
weight[0] = 0

while true {
    guard let now = heap.deheap() else { break }
    if now.w > weight[now.d] { continue }
    if now.d == nm[0] - 1 {
        result = weight[now.d]
        break
    }
    
    for edge in edges[now.d] {
        if bungi[edge.d] == 1 {
            if edge.d != nm[0] - 1 { continue }
        }
        
        let newWeight = edge.w + now.w
        if newWeight >= weight[edge.d] { continue }
        weight[edge.d] = newWeight
        heap.enheap((edge.d, newWeight))
        
    }
}
print(result)

